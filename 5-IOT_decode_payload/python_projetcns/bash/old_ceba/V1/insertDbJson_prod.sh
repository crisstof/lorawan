...
project=$2
project_schema=`psql -q -h cebabdd01 -X -U cebauser -d datastorage -t -c "SELECT schema_name FROM information_schema.schemata WHERE schema_name = '$project'"`
...
psql -q -h cebabdd01 -X -U cebauser -d datastorage -t -c "CREATE TABLE IF NOT EXISTS $project.json_file(id serial UNIQUE,file_name TEXT,app_name TEXT,upload_state BOOLEAN,PRIMARY KEY(file_name,app_name))"

#Recuperation de la liste des application Name dans la table correspondante
appNameList=`psql -h cebabdd01 -X -U cebauser -d datastorage -t -c "SELECT DISTINCT app_name FROM $project.json_file"`
...
then
	...
	do
		...
		then
			...
			#Recupération de la liste des fichiers déjà en base et qui ont été inserés correctement !
        		DbFilesList=`psql -h cebabdd01 -X -U cebauser -d datastorage -t -c "SELECT file_name from $project.json_file where app_name='$appName' and upload_state = true"`
			...
			then
				...
				psql -q -h cebabdd01 -X -U cebauser -d datastorage -t -c "INSERT INTO $project.json_file(file_name,app_name,upload_state) VALUES('$nomFichier','$appName','false')"

                        	#Recuperation de l'ID du nom du fichier JSON
                        	id=`psql -h cebabdd01 -X -U cebauser -d datastorage -t -c "SELECT id FROM $project.json_file WHERE file_name='$nomFichier' AND app_name='$appName'"`
				...
				tableName="Json_"$appName"_row"
                        	...
                        	then
        	        	        echo "CREATION D'UNE TABLE POUR $appName"
                	       		...
                	       		psql -q -h cebabdd01 -X -U cebauser -d datastorage -t -c "CREATE TABLE $project.$tableName (line_number INTEGER, data jsonb NOT NULL, file_id INTEGER, PRIMARY KEY(line_number, file_id))"
                        		psql -q -h cebabdd01 -X -U cebauser -d datastorage -t -c "ALTER TABLE $project.$tableName ADD CONSTRAINT json_file_id_fk FOREIGN KEY(file_id) REFERENCES $project.json_file(id) MATCH FULL"
				...
				psql -q -h cebabdd01 -X -U cebauser -d datastorage -f /home/loraserver/requetes.sql
			
				#Mise à jour de l'état de l'upload du fichier JSON
				psql -q -h cebabdd01 -X -U cebauser -d datastorage -t -c "UPDATE $project.json_file SET upload_state = 'true' WHERE id = '$id'"		
			...
			psql -q -h cebabdd01 -X -U cebauser -d datastorage -t -c "CREATE TABLE IF NOT EXISTS $project.json_file(id serial UNIQUE,file_name TEXT,upload_state BOOLEAN,PRIMARY KEY(file_name))"
       	 		psql -q -h cebabdd01 -X -U cebauser -d datastorage -t -c "CREATE TABLE IF NOT EXISTS $project.json_row(line_number INTEGER, data jsonb NOT NULL, file_id INTEGER, PRIMARY KEY(line_number, file_id), FOREIGN KEY(file_id) REFERENCES $project.json_file(id) MATCH FULL)"
        		
			#Recupération de la liste des fichiers déjà en base
                        DbFilesList=`psql -h cebabdd01 -X -U cebauser -d datastorage -t -c "SELECT file_name from $project.json_file"`
                        ...
                        then
                                ...
                                #Insertion du nom du fichier JSON dans la table correspondante
                                psql -q -h cebabdd01 -X -U cebauser -d datastorage -t -c "INSERT INTO $project.json_file(file_name,upload_state) VALUES('$nomFichier','false')"

                                #Recuperation de l'ID du nom du fichier JSON
                                id=`psql -h cebabdd01 -X -U cebauser -d datastorage -t -c "SELECT id FROM $project.json_file WHERE file_name='$nomFichier'"`
                                ...
                                while read -r line; do
                                        ...
                                        #Insertion dans la table json correspondante
                                        echo "INSERT INTO $project.json_row(line_number, data, file_id) VALUES($cpt, '$line', $id);" >> /home/jemezhou/requetes.sql
